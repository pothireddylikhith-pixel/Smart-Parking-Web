...
// Generate Indian License Plate Format (6 to 9 alphanumeric chars)
function generateRandomPlate() {
  const states = ['DL', 'MH', 'KA', 'TS', 'TN', 'HR', 'UP'];
  const stateCode = states[Math.floor(Math.random() * states.length)];
  const districtCode = String(1 + Math.floor(Math.random() * 9)); // 1 digit
  
  const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const lettersCount = Math.random() < 0.5 ? 1 : 2; // 1 or 2 letters
  let letters = '';
  for (let i = 0; i < lettersCount; i++) {
    letters += alphabet[Math.floor(Math.random() * 26)];
  }
  
  const numbersCount = 3 + Math.floor(Math.random() * 2); // 3 or 4 numbers
  let numbers = '';
...
function handleSpotSelection(spot) {
  ...
  if (spot.state === 'vacant') {
    ...
    btnSubmit.disabled = isLotLocked;
...
function calculateBookingCost() {
  ...
  // Apply Admin Tariff Surge Multiplier
  total = total * tariffMultiplier;
  baseRate = baseRate * tariffMultiplier;
  premium = premium * tariffMultiplier;
...
function handleBookingSubmit() {
  ...
  if (spot.state === 'vacant') {
    if (isLotLocked) {
      showToast('Lockdown Active', 'This facility is currently under emergency lock. No new reservations allowed.', 'error');
      return;
    }
    ...
    // Enforce 6 to 9 alphanumeric characters validation constraint
    const cleanedPlate = plateInput.replace(/[^A-Za-z0-9]/g, '');
    if (cleanedPlate.length < 6 || cleanedPlate.length > 9) {
      showToast('Invalid Plate', 'Plate must contain between 6 and 9 alphanumeric letters/digits.', 'error');
      return;
    }
...
// Background simulator nationwide process (runs every 5s)
...
// Whitelist of authorized system administrators
const AUTHORIZED_ADMINS = [
  'pothireddylikhith@gmail.com'
];
...
function handleLogin(e) {
  ...
  // Restrict Admin Portal Logins to Whitelist
  if (roleInput === 'admin') {
    const isAuthorized = AUTHORIZED_ADMINS.includes(emailInput.toLowerCase());
    if (!isAuthorized) {
      showToast('Access Denied', 'This email is not authorized for System Administrator access.', 'error');
      return;
    }
  }
...
function checkActiveSession() {
  ...
  if (cachedSession) {
    try {
      ...
      // Safety check: Validate cached admin sessions
      if (session.role === 'admin' && !AUTHORIZED_ADMINS.includes(session.email.toLowerCase())) {
        storage.removeItem('omnipark_active_session');
        return;
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Admin Controller Event Listeners ---
  
  // Tariff slider adjustments
  document.getElementById('admin-tariff-modifier').addEventListener('input', (e) => {
    tariffMultiplier = parseFloat(e.target.value);
    document.getElementById('admin-tariff-multiplier-val').innerText = `${tariffMultiplier.toFixed(1)}x`;
    calculateBookingCost();
  });

  // Emergency lockdown trigger
  document.getElementById('btn-admin-emergency-lock').addEventListener('click', () => {
    isLotLocked = !isLotLocked;
    const btn = document.getElementById('btn-admin-emergency-lock');
    const text = document.getElementById('btn-admin-lock-text');
    
    if (isLotLocked) {
      btn.style.background = 'rgba(239, 68, 68, 0.2)';
      text.innerText = "Lot Frozen (Unlock)";
      logActivity("EMERGENCY LOCKDOWN ACTIVATED BY ADMINISTRATOR.", "out");
      showToast("Emergency Freeze", "Garage entry/exit systems frozen.", "error");
      
      // Visually lock spots
      document.querySelectorAll('.parking-spot.state-vacant').forEach(spot => {
        spot.style.opacity = '0.35';
      });
    } else {
      btn.style.background = 'rgba(239, 68, 68, 0.03)';
      text.innerText = "Emergency Lot Lockdown";
      logActivity("Emergency lockdown lifted by admin.", "in");
      showToast("System Unlocked", "Standard services resumed.", "success");
    ...
    // Disable confirm button if vacant selected
    const btnSubmit = document.getElementById('btn-submit-booking');
    if (selectedSpotId && currentHub) {
      const spot = currentHub.spots.find(s => s.id === selectedSpotId);
      if (spot && spot.state === 'vacant') {
  ...
  // Instant fill trigger
  document.getElementById('btn-admin-instant-fill').addEventListener('click', () => {
    if (!currentHub) return;
    let filledCount = 0;
    currentHub.spots.forEach(spot => {
      if (spot.state === 'vacant') {
        const durationHrs = 1 + Math.floor(Math.random() * 4);
        spot.state = 'occupied';
        spot.plate = generateRandomPlate();
        spot.duration = durationHrs;
        spot.bookedAt = Date.now();
        spot.expireAt = Date.now() + (durationHrs * 3600 * 1000);
        spot.carColor = CAR_COLORS[Math.floor(Math.random() * CAR_COLORS.length)];
        ...
        const fee = spot.type === 'ev' ? 120.00 : 80.00;
        currentHub.revenue += fee;
        addTransaction(`Admin Auto-fill Spot ${spot.id}`, currentHub.name, fee, 'booking');
        filledCount++;
      }
    });
    if (filledCount > 0) {
      logActivity(`Admin action: Instantly filled ${filledCount} vacant spots.`, 'warn');
      showToast('Stalls Filled', `All ${filledCount} vacant bays occupied.`, 'warning');
      saveToDatabase();
      renderMap();
      updateTelemetry();
      updateRevenueDisplay();
  ...
  // Instant clear trigger
  document.getElementById('btn-admin-instant-clear').addEventListener('click', () => {
    if (!currentHub) return;
    let clearedCount = 0;
    currentHub.spots.forEach(spot => {
      if (spot.state !== 'vacant') {
        ...
        spot.expireAt = null;
        spot.carColor = null;
        addTransaction(`Admin cleared Spot ${spot.id}`, currentHub.name, 0, 'release');
        clearedCount++;
      }
    });
    if (clearedCount > 0) {
      logActivity(`Admin action: Instantly cleared ${clearedCount} spots.`, 'in');
      showToast('Stalls Vacated', `All ${clearedCount} occupied bays cleared.`, 'info');
      saveToDatabase();
      renderMap();
      updateTelemetry();
      updateLocalAnalyticsTick();
